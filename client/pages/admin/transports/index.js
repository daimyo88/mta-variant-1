...
export default function Page() {
    ...
    const { data, mutate } = useSWR(
        user?.data && canFetch && "/api/ships?p="+ page +"&pp="+itemsPerPage +"&s=" + search, fetcher);

    const ships = data?.data?.data?.[0]?.ships;
    ...
    return (
        <Admin>  
            ...
            { !ships && <div style={{position:'relative', minHeight: '300px'}}>
                 <Loader />
            </div> }

            { !!ships?.length && <TableContainer style={{flexGrow: 1}}>
                    ...
                    {ships?.map((item) => {
                        return(
                            <TableRow className={classes.row} key={item._id}>
                                <TableCell>
                                    <Link href={'/admin/ships/' + item._id }>
                                ...
                                {/* <TableCell>
                                    <b className={classes.link} onClick={() => { setSearch(item.shipCategory)}}>
                                        { item.shipCategory }
                                    </b>
                                </TableCell> */}
                                <TableCell>
                                    <b className={classes.link} onClick={() => { setSearch(item.productGroup)}}>
                                        {item.productGroup ? t(`translation:${item.productGroup}`) : ''}
                                    </b>
                                </TableCell>
                                { user.data?.role === 'admin' && <TableCell>
                                    <Link href={'/admin/users/' + item.shipOwner._id }>
                                        <a style={{fontWeight:'bold'}} className={classes.link}>
                                            { item.shipOwner.fullname }
                                        </a>
                                    </Link>
                                </TableCell> }
                                <TableCell align="right">
                                    ...
                                    <Tooltip arrow title={t('translation:delete')}>
                                        <IconButton className={classes.icon} size="small" onClick={() => {setCurrentItem({_id: item._id, name: item.title });setDeleteModal(true)}}>
                                            <DeleteIcon />
                                        </IconButton> 
                                    </Tooltip> 
                                </TableCell>
            ...
            <Modal 
                ...
                text= {t('modals:default-prefix') + t('modals:delete-ship', { name: currentItem?.name })}
            />
        </Admin>
    )
}